...
/// F1: provide_liquidity mints shares and transfers tokens.
#[test]
fn f1_provide_liquidity_mints_shares() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    token_admin_client.mint(&lp, &1000i128);
    ...
    assert_eq!(shares, 500, "first LP gets 1:1 shares");
    assert_eq!(token.balance(&lp), 500, "tokens transferred from LP");
    assert_eq!(token.balance(&contract_id), 500, "contract holds LP tokens");
}

/// F2: Second LP gets proportional shares.
#[test]
fn f2_second_lp_gets_proportional_shares() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    token_admin_client.mint(&lp1, &1000i128);
    token_admin_client.mint(&lp2, &500i128);
    ...
    let shares1 = client.provide_liquidity(&lp1, &pool_id, &1000i128);
    // lp2 deposits half as much → gets half the shares (1000 shares exist, deposit 500, total_dep 1000 → 500 shares)
    let shares2 = client.provide_liquidity(&lp2, &pool_id, &500i128);
    assert_eq!(shares1, 1000);
    assert_eq!(shares2, 500);
}

/// F3: withdraw_liquidity returns principal + rewards.
#[test]
fn f3_withdraw_liquidity_returns_principal_and_rewards() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    token_admin_client.mint(&lp, &1000i128);
    token_admin_client.mint(&bettor, &200i128);
    ...
    let shares = client.provide_liquidity(&lp, &pool_id, &1000i128);

    // Bet to generate fees
    client.place_bet(&bettor, &pool_id, &0, &200);
    env.ledger().with_mut(|l| l.timestamp = 3601);
    client.settle_pool(&creator, &pool_id, &0);
    client.claim_winnings(&bettor, &pool_id); // accrues LP fee

    // LP must have pending rewards
    let info = client.get_liquidity_info(&pool_id, &lp).expect("LP info must exist");
    assert!(info.pending_rewards > 0, "LP must have accrued rewards");
    assert_eq!(info.shares, shares);
    assert_eq!(info.deposited, 1000);

    let lp_balance_before = token.balance(&lp);
    let total_out = client.withdraw_liquidity(&lp, &pool_id, &shares);
    assert_eq!(total_out, info.deposited + info.pending_rewards);
    assert_eq!(token.balance(&lp), lp_balance_before + total_out);

    // Position must be cleared
    assert!(client.get_liquidity_info(&pool_id, &lp).is_none());
}

/// F4: LP can top up an existing position; settled rewards are preserved.
#[test]
fn f4_lp_topup_preserves_settled_rewards() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let lp = Address::generate(&env);
    let bettor = Address::generate(&env);
    token_admin_client.mint(&lp, &2000i128);
    token_admin_client.mint(&bettor, &200i128);
    ...
    // First deposit
    client.provide_liquidity(&lp, &pool_id, &1000i128);

    // Generate some fees
    client.place_bet(&bettor, &pool_id, &0, &200);
    env.ledger().with_mut(|l| l.timestamp = 3601);
    client.settle_pool(&creator, &pool_id, &0);
    client.claim_winnings(&bettor, &pool_id);

    let info_before = client.get_liquidity_info(&pool_id, &lp).unwrap();
    let rewards_before = info_before.pending_rewards;
    assert!(rewards_before > 0);

    // Top up — previous rewards must be preserved as settled_rewards
    client.provide_liquidity(&lp, &pool_id, &500i128);

    let info_after = client.get_liquidity_info(&pool_id, &lp).unwrap();
    // Pending rewards after top-up should still reflect accumulated earnings
    assert_eq!(info_after.pending_rewards, rewards_before, "rewards preserved on top-up");
}

/// F5: Partial share withdrawal works correctly.
#[test]
fn f5_partial_share_withdrawal() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    client.withdraw_liquidity(&lp, &pool_id, &(shares / 2));

    // Position must still exist with remaining shares
    let info = client.get_liquidity_info(&pool_id, &lp).expect("position must remain");
    assert_eq!(info.shares, shares - shares / 2);
}
